...
public class AnalizadorSintactico2 {
    ...
    public AnalizadorSintactico2(AnalizadorLexico analizadorLexico) {
    ...
    // registra un error semántico y continúa el análisis
    private void errorSemantico(String mensaje, int linea) {
        erroresSemanticos.add("Error semántico en línea " + linea + ": " + mensaje);
    }
    ...
    public void programa() {
        ...
        if (erroresSemanticos.isEmpty()) {
            System.out.println("Programa sintáctica y semánticamente correcto.");
        } else {
            System.out.println("Programa sintácticamente correcto, pero con errores semánticos:");
        ...
        for (Token idTok : identificadores) {
            ...
            if (!ambitoActual.insertar(simbolo)) {
                errorSemantico("El identificador '" + idTok.getLexema()
                        + "' ya había sido declarado en este ámbito", idTok.getLinea());
            }
        }
    }
    ...
    private void chequearTipoCondicion(TipoDato tipoCondicion, int linea) {
        if (tipoCondicion != null && tipoCondicion != TipoDato.BOOLEAN) {
            errorSemantico("La condición debe ser de tipo BOOLEAN y es " + tipoCondicion, linea);
        }
    }
    ...
    private void validarAsignacion(Simbolo simbolo, Token idTok, TipoDato tipoExpresion) {   
        ...
        if (simbolo.getCategoria() != CategoriaSimbolo.VARIABLE
            ...
            errorSemantico("Tipos incompatibles en la asignación a '" + idTok.getLexema()
                    + "' (se esperaba " + simbolo.getTipo() + " y se recibió " + tipoExpresion + ")",
                    idTok.getLinea());
        }
    }
        ...
        for (int i = 0; i < parametros.size(); i++) {
            ...
            if (tipoRecibido != null && tipoEsperado != tipoRecibido) {
                errorSemantico("El argumento " + (i + 1) + " de '" + subprograma.getNombre()
                        + "' debe ser " + tipoEsperado + " y se recibió " + tipoRecibido, lineaLlamada);
            }
        }
    }
    ...
    private void comandoLectura() {
        ...
        if (simbolo == null) {
            errorSemantico("El identificador '" + idTok.getLexema() + "' no fue declarado", idTok.getLinea());
                ...
                && simbolo.getCategoria() != CategoriaSimbolo.PARAMETRO) {
            errorSemantico("Solo se puede leer sobre variables o parámetros, no sobre '"
    ...
    private TipoDato expresion() {
        ...
        if (esOperadorRelacional(preanalisis.getTipo())) {
            ...
            if (tipoIzquierdo != null && tipoDerecho != null && tipoIzquierdo != tipoDerecho) {
                errorSemantico("Operandos de tipos incompatibles en la comparación ("
                ...
                if (preanalisis.getTipo() == TipoToken.PARENTESIS_ABRE) {
                    ...
                    if (simbolo != null) {
                        if (simbolo.getCategoria() != CategoriaSimbolo.FUNCION) {
                            errorSemantico("'" + idTok.getLexema() + "' no es una función", idTok.getLinea());
                        } else {
    ...
    private void declaracionProcedimiento() {
        ...
        if (!ambitoActual.insertar(simbolo)) {
            errorSemantico("El identificador '" + nombreTok.getLexema()
                    + "' ya había sido declarado en este ámbito", nombreTok.getLinea());
        }
        ...
        for (GrupoParametros grupo : grupos) {
            for (Token idTok : grupo.identificadores) {
                if (subprograma.getTablaLocal().buscarLocal(idTok.getLexema()) != null) {
                    errorSemantico("El parámetro '" + idTok.getLexema() + "' ya había sido declarado",
                            idTok.getLinea());
                } else {
    ...
    public static void main(String[] args) {
        ...
        if (args.length != 1) {
            System.out.println("Uso: java AnalizadorSintactico2 archivo.pas");
            return;
        }
